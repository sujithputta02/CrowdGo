...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError('Passwords do not match. check your flow.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      setError(err.message || 'Failed to join. Aura too low.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError('Google sync failed. Manual entry required.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[60vw] h-[60vw] bg-secondary/10 rounded-full blur-[150px] animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-primary/10 rounded-full blur-[120px]" />
      </div>
      ...
      <main className="w-full max-w-md relative z-10">
        ...
        >
          <div className="text-center mb-8">
            <Link href="/login" className="inline-flex items-center gap-2 text-text-muted hover:text-white transition-colors text-xs font-black uppercase mb-6 group">
               <ChevronLeft size={16} className="group-hover:-translate-x-1 transition-transform" />
               Back to Login
            </Link>
            <h1 className="text-4xl font-black mb-2 tracking-tighter">JOIN THE FLOW.</h1>
            <p className="text-text-muted text-sm italic">Unlock the ultimate stadium experience.</p>
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-6">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-5 bg-secondary text-white rounded-2xl font-black text-lg shadow-2xl shadow-secondary/30 flex items-center justify-center gap-3 active:scale-[0.98] transition-all disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-secondary/50 focus:ring-offset-2 focus:ring-offset-background"
              aria-busy={loading}
            >
          ...
          <div className="mt-8 flex justify-center items-center gap-2 opacity-30">
            <ShieldCheck size={14} />
            <span className="text-[10px] font-black uppercase tracking-[0.2em]">Verified Identity Secure</span>
          </div>
        </motion.div>
      </main>
    </div>
